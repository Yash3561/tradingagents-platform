...
def build_grid(quick: bool = False) -> list[IntradayPolicy]:
    ...
    for orm, vr, st, rr, hold in product([15, 30], [1.0, 1.5], stops, rrs, holds):
        pols.append(IntradayPolicy(setup="orb", or_minutes=orm, vol_ratio_min=vr,
                                   stop_atr_mult=st, rr=rr, max_hold_bars=hold))
    for dev, rmax, st, rr, hold in product([1.5, 2.0, 2.5], [35.0, 100.0],
                                           stops, rrs, holds):
        pols.append(IntradayPolicy(setup="vwaprev", dev_entry_atr=dev, rsi_max=rmax,
                                   stop_atr_mult=st, rr=rr, max_hold_bars=hold,
                                   above_vwap=False))
    for vr, av, st, rr, hold in product([1.0, 1.5], [True, False], stops, rrs, holds):
...
def entry_signal(ts: TickerSeries, pol: IntradayPolicy) -> np.ndarray:
    ...
    late = np.array([t >= NO_ENTRY_AFTER for t in ts.time])
    ok &= ~late
    if pol.setup == "orb":
...
def run_tournament(quick: bool = False) -> dict:
    bars = load_bars()
    ...
    # rolling folds over the working window: last 4×TEST_DAYS are test slices
    n_folds = 4
    folds = []
    ...
    print(f"[intraday] {len(series)} tickers, {len(all_days)} sessions, "
          f"{len(grid)} policies, {n_folds} folds + {HOLDOUT_DAYS}d holdout")
    ...
    report = {
        ...
        "caveats": [
            "60 trading days of 5m bars — a single market regime",
    ...
    out = Path("/tmp/intraday_report.json")
