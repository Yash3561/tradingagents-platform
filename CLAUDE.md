...
> Last updated: 2026-07-25 (User caught a real bug live in the admin account: Day P&L showed +$100,012.80, identical to total equity. Root cause duplicated across EIGHT call sites — `last_equity = acct.get("last_equity", equity)` only substitutes a default when the key is MISSING, but Alpaca returns an actual `0` for a freshly-created/reset account, and `equity - 0` is just equity. Added `core/pnl.py::compute_day_pnl()` — one shared function instead of eight copies of the same math — wired into dashboard.py/analytics.py/overnight_agent.py/equity_tracker.py/portfolio.py; equity_tracker.py was the important one since it was writing the bad number into `equity_snapshot` every 15 minutes, not just displaying it. Also fixed the scan-watchdog's real false-positive pattern (flagged 2026-07-20 as a "silent outage" that never happened): it checked `track-record.recent`, hard-capped at 20 rows platform-wide for the public page's display — a busy Agents-engine day can push a real same-day trade from a different account out of that window before the evening check runs. Added `analyses_today` (uncapped full-day count) to the public payload, watchdog now checks that instead. Shipped: Trade History "Market Context" section (`GET /trades/{id}/market-context`, real exchange volume + day price range per trade, options resolve to their underlying since Alpaca has no per-contract daily-volume history); public `/track-record/methodology` page (also fixed a real accuracy problem it surfaced — the "Total Analyses" KPI was labeled "full multi-agent debates" but actually counts all 6 engines including 5 zero-LLM ones); a dedicated Options Policy settings panel (`pead_options_*` sliders — every other engine had one, this didn't); `INCIDENTS.md` (SECRET_KEY rotation breaks every stored broker connection platform-wide, Render free-tier spin-down kills the scheduler since it's in-process not a separate worker, the watchdog's own false-positive history, Yahoo IP-rate-limiting pattern). Insider-buying research went from an 8-ticker manual pilot to a real walk-forward tournament: parallelized `insider_data.py`'s per-filing fetch (6-worker pool, BAC worst-case 410s→107s — was impractical to scale past a handful of tickers before this), fixed two entity-filter gaps a 150-ticker scan surfaced (punctuated suffixes like "INC." and "L.L.C." weren't matching the exact-string check, letting "SUMITOMO MITSUI FINANCIAL GROUP, INC." and a Google Ventures fund entity slip through as if they were real individuals; multi-share-class tickers like GOOG/GOOGL/GOOGM/GOOGN share one CIK and were quadruple-counting the same real transaction — added `dedupe_tickers_by_cik()`). Corrected wide-scan result: 122 real purchase events across 31 of 150 tickers in a trailing year. New `research/insider_buying.py` (`SIGNAL_UNIVERSE` = those 31 names, NOT the general 59-name UNIVERSE — explicitly documented as a second selection-bias layer on top of every tournament's usual survivorship bias) ran its first walk-forward: winner is `min_distinct_buyers=1` (a single large purchase, $50k+ notional) — the clustering hypothesis that looked strongest by eye (MRVL 4-execs-same-day, LLY 5-execs-one-week) actually LOST to the simpler signal once tested out-of-sample. Holdout: Sharpe 1.49, +8.66% return, -3.93% maxdd (SPY holdout +17.27%) — but only ONE fold (shorter 2021+ data window than other tournaments), and the overfit gap is inverted (train Sharpe -0.69, test +2.87) which a single fold can't distinguish from luck. Explicit verdict: "worth a forward-paper test," NOT validated enough to become a 7th engine — widening the history window further back for real multi-fold coverage is a deliberate future decision, not something to just re-run. Momentum had a real drawdown mid-session (+4.84%/Sharpe 4.22 → -9.59% maxdd) — user initially believed they'd instructed a manual close ("book the profits"); checked the full conversation, no such instruction existed. Explicit decision after discussing the tradeoff: leave it running exactly as designed, no manual intervention — Momentum has no stop/target by design, exits only via monthly rotation, and manually overriding it would make the whole backtest validation meaningless.)
> Previous session: 2026-07-22 (Sixth engine went live end-to-end: PEAD Options (`agents/earnings_pead_options.py`, `strategy_mode=earnings_options`) got its own dedicated account — yashchaudhary3561@gmail.com, Alpaca `PA3OOT9IK85L`. First real trade: CCK $125C exp 8/21, 14 contracts @ $3.50 (~4.9% of account, sizing correct) — closed same-day via stop_loss at a REAL -87.14% / -$4,270 loss, overshooting the intended -60% max_loss_pct stop. Rollout surfaced and fixed FOUR real production bugs same day, all deployed+smoke-tested (commits 8c25feb→72edbff): (1) yfinance fallback calls in `core/market_data.py` had no timeout, unlike every other call in that file — stalled the whole-market earnings prescreen 6+ minutes under Render's known Yahoo-IP throttling; added `_yf_bounded()`, hard 15s ceiling. (2) `earnings_whole_market` was never in platform `DEFAULTS` — only ever hand-set per-user for the original seemplyai PEAD-stock account, so every OTHER earnings-mode account (including this brand-new one) silently used the narrower top-150-market-cap watchlist instead of "who actually reported earnings today"; now a real default. (3) Scanner page frontend crashed (`undefined.toLocaleString`) the first time ANY earnings-mode scan ever completed and returned results — the technical-prescreen table assumed every engine returns RSI/MACD/price fields that a PEAD candidate (`{ticker,direction,score,reason}`) doesn't have; now renders em-dashes instead of crashing for any zero-technical-score engine. (4) Trade History showed -8714.29% instead of the real -87.14% for options trades — `pnl/(fill_price*qty)` forgets options trade in 100-share contracts; fixed via OCC-symbol detection (`CCK260821C00125000` etc.). Separately, a real risk fix: options exits went through Alpaca's blind market-order `close_position` (same path as equities) — on a thin single-name option that's what let the stop overshoot -60% to -87% real; now closes via a marketable limit 5% through the last-known quote (commit 532c20f). Deliberately left the shared 60s position_monitor poll interval untouched — global infra, bigger change than this called for. Confirmed the next day: account quiet, no new trades past the CCK loss, scan health normal. Also shipped: first-run interactive guided tour + new `/how-it-works` page (`components/onboarding/GuidedTour.tsx`, custom SVG-spotlight overlay — no tour library exists in this codebase and none was added; persistence reuses the existing `user_settings` store via one new `onboarding_tour_completed` DEFAULTS key, zero new endpoints) — verified live in prod on the real admin account after three real bugs found by actually clicking through all 8 steps (card could overflow the viewport bottom; the Intelligence nav group sits below the sidebar's own scroll fold so its spotlight rect computed off-screen; centered steps were never actually centered because Framer Motion silently owns `transform` once `scale` is animated, clobbering a manual `translate(-50%,-50%)` — fixed by splitting positioning onto a plain non-animated wrapper). Landing page (logged-out) previously only described the AI Agents engine — added a "Six engines" section so the five zero-LLM engines are visible pre-signup too; new `docs/getting-started-guide.md` (signup, free Alpaca paper-key steps, engine picker, FAQ) for sharing outside the app. Known cosmetic bug, not yet fixed: Dashboard's System Control card sometimes shows a stale/wrong "N positions" figure unrelated to the logged-in account's real position count.)
> Previous session: 2026-07-20 (First fully-healthy live day. Two new dedicated accounts live: Momentum (yashchaudhary3581@gmail.com, strategy_mode=momentum, first rotation AMD/INTC/LRCX/MU filled ~25/22/28/20% weights, day 1 +1.02%) and PEAD Aggro (yashchaudhary3591@gmail.com, earnings whole-market, 25% sizing, $1B cap floor, first trade TFC @ 25%). Found+fixed a real bug via Render logs: researcher-debate NIM calls hit intermittent 404s (JPM/MCD/NFLX failed same scan GS/ASML succeeded) — 404 added to the transient-retry set in structured_runner.py._nim_structured (was previously untreated as permanent). Watchlist expansion: Yash+Quant's static 39-ticker WATCHLIST replaced by `core/market_data.py::nasdaq_market_universe()` — NASDAQ public screener, top-150 by market cap, daily-cached, parallelized 6-worker prescreen; falls back to the old static list if the fetch fails. New signal: `intraday_volume_imbalance()` — real up/down-tick volume from today's 5min Alpaca bars, wired into Sentiment analyst (fills a previously-always-null field honestly) + Technical analyst (full market_data dump) + Quant scorer (±6). New read-only monitoring surface: `/api/v1/monitor/accounts-summary` (own auth — `require_monitoring_key` header, MONITORING_API_KEY env, fails closed if unset — NOT the JWT path, for unattended agents with no login). Hourly cloud routine "TradingAgents Platform Health Monitor" (claude.ai/code/routines/trig_01EheikbuqVa99eGKpCt64LR) checks backend health + per-account equity/scan_enabled/failed-runs + GH Actions status, emails ygc2@njit.edu ONLY on a real problem via the Gmail MCP connector. Contracts loosened: `RiskAssessment.recommended_position_pct` / `FinalDecision.position_size_pct` le raised 20→40 (was a latent crash waiting for momentum's ~25-38% weights AND the 25% PEAD aggression arm — same bug, two engines). Nightly DB backup CONFIRMED running green in prod (not just configured). Nothing here is real money — all 5 accounts are Alpaca paper; that gap was explicitly re-clarified with the user this session, don't let future conversation momentum blur it.)
> Previous session: 2026-07-18 (Momentum tournament DONE — `app/research/momentum.py`, 288 policies, report `docs/research/momentum-walkforward-2026-07-18.{md,json}`. Winner mom[126-0]top4 rb21d inv_vol: test Sharpe 1.29 all-folds-positive, holdout +132.7% — BUT out-of-universe validation shows the truth: FAILS cold on a 76-name defensive universe (+2% vs +26% EW), PASSES on the 135-name union (+606% test era vs +238% EW — ranking finds leaders by itself). Test-era Sharpe 0.93 vs 0.91 for union EW = selection is real, risk-adjusted alpha ≈ zero, extra return is concentration; survivorship flatters this family worst of all. Verdict: forward-paper-arm candidate, not a validated edge. Also shipped: resend-verification no longer lies on provider failure (Brevo error body logged, 502 surfaced, banner shows errors — the "no verification email" report was a false alarm: quanttest@example.com can't receive mail); Sentry backend init (needs SENTRY_DSN on Render); weekly research tournament GitHub Action (Sat 13:00 UTC, needs ALPACA_API_KEY/SECRET repo secrets); RiskAssessment.rejection_reason now strips <think> blocks + caps 400 chars; research momentum --quick can no longer touch the holdout. Local research venv works: system Python 3.14 + pandas/yfinance venv, PYTHONPATH=backend, CWD=repo root for .env.)
> Previous session: 2026-07-17 evening (CRITICAL session, full outage post-mortem: scheduled scans silently dead since 7/15. THREE stacked root causes, all fixed+deployed: (1) Render env Alpaca keys were the ygc2 account's PRE-RESET pair — 401 on /v2/clock read as "market closed" every tick; keys rotated everywhere by user, plus scheduler clock-error now falls back to local ET hours instead of standing down; (2) scheduler fragility — 5-min scan windows, no timeouts, dedupe flag burned before the scan ran; now 15s timeouts, wide catch-up windows (9:35–11:30 / 13:00–15:00 ET), Redis once-per-day guard released on failure, heartbeats; (3) Yahoo rate-limited Render's shared egress IP — every yfinance call 429'd, scans starved even when they fired. NEW `core/market_data.py`: Alpaca-first bars/snapshot, NASDAQ-first earnings surprises (calendar rows cached per-process = timing memory; past dates lose the pre/post field, yfinance timestamp rescue), yfinance = fallback only; fundamentals isolated so Yahoo failure degrades instead of blocking. Earnings blackout breaker exempted for engine=earnings-pead (was vetoing every PEAD order). Earnings arm LIVE on seemplyai@gmail.com: whole-market pool (150 reporters, ≥$2B), 25 candidates/scan (zero-LLM cap; agents keeps 10), 10% sizing (bound raised to 25 for a future dedicated aggression arm — do NOT raise the clean arm). Scan watchdog GitHub Action (no secrets) + nightly encrypted pg_dump workflow (needs NEON_DATABASE_URL + BACKUP_PASSPHRASE secrets). Missed UNH +29.8% and TSM +11.4% entries 7/16 to the outage. Precision backtest at exact live sizing: docs/research/earnings-live-config-precision-2026-07-17.json — holdout +15.4%, ~$835/mo mean on $100k, NEGATIVE median month (few big months carry the year — do not panic-kill flat months). Next live test: Monday 2026-07-20 09:35 ET.)
...
**Six strategy engines** selectable per user via `strategy_mode` setting:
...
- `momentum` — monthly top-N relative-momentum rotation, zero LLM (`agents/momentum_rotation.py`, shipped 2026-07-18, defaults = tournament winner mom[126-0]top4 inv_vol @95% exposure). Rides scheduled scans; Redis `momentum:last_rebalance:{user_id}` marks the cycle (advanced only after a successful rotation → failed attempts retry next window; rebalance due = 21 TRADING days counted off the bar index). Ranking math is imported from `research/momentum.py::latest_target_weights` (live/backtest parity verified bit-exact). One AgentRun per rotation order (llm_model="momentum-rotation"). ⚠ Deliberate deviations wired into shared code: NO stop/target — plain market orders, not brackets (`_place_order_if_approved` branches on engine); `position_monitor` SKIPS engine-tagged positions (else its 7%/15% defaults would dismantle the book); SELL rotation exits bypass `long_only`; exempt from earnings blackout (holds through earnings by design). Single-name weight capped 40% of book; contracts position bounds raised le=20→40 (was a latent crash for the 25% PEAD aggression arm too). DEDICATED account only — it rotates every universe position it sees. Universe = research UNIVERSE (59 names), settings `momentum_*` (NUMERIC_BOUNDS/ENUM_VALUES-clamped). Judge vs equal-weight universe benchmark, NOT absolute return (survivorship + concentration — see research verdict).
...
- `earnings_options` — same PEAD entry gate as `earnings` (identical `check_recent_earnings_surprise`, identical whole-market pool via `earnings_whole_market`), expressed as a defined-risk long call instead of stock (`agents/earnings_pead_options.py`, shipped 2026-07-21). Uses Alpaca's native options API (`core/alpaca_options.py` — real OCC contract symbols, real bid/ask, real computed delta/IV for liquid strikes, no Black-Scholes needed) — no yfinance dependency at all. `pick_contract()` targets a delta/expiry (`pead_options_target_delta`/`target_days`, defaults 0.35 / 14d), exits on `pead_options_target_gain_pct`/`max_loss_pct` (100%/60%) or a `hold_days` time exit, all enforced by `position_monitor.py`'s existing loop (options positions get their own branch there — premium-based target/max-loss, multiplier-aware `pnl_dollar`). Exits go through `AlpacaClient.submit_options_order` as a marketable limit (5% through the last-known quote), NOT the blind-market-order `close_position` equities use — a thin single-name option let an early stop overshoot on 2026-07-21, this is the fix. AgentRun rows tagged `llm_model="earnings-pead-options"`. **LIVE since 2026-07-21 on yashchaudhary3561@gmail.com** (Alpaca `PA3OOT9IK85L`, DEDICATED account): scan_enabled=true, 5% position size. Settings `pead_options_*`, all NUMERIC_BOUNDS-clamped — no dedicated UI slider panel yet (only the Strategy Engine dropdown), tune via the settings API if needed.
...
1. ✅ **DB backups** — nightly Neon `pg_dump` (GitHub Action → private artifact). CONFIRMED running green in prod as of 2026-07-20 (not just configured).
2. 🟡 **Error alerting** — Sentry backend init code shipped 2026-07-18 (`sentry_dsn` setting, gated). Still needs a real `SENTRY_DSN` value set on Render — code alone does nothing until that env var exists. Frontend Sentry not started.
3. ✅ **Operator kill switch** (2026-07-20) — `trading_halted` platform_settings flag, `GET/POST /admin/trading-halt`. Checked at 3 independent points: `scanner.run_market_scan` (before any scan work, covers scheduled + manual Scan Now), `structured_runner._order_seatbelt_blocked` (final line of defense at order placement), `intraday_engine._cycle`'s entry gate (the always-on RTH loop bypasses the scanner entirely). Deliberately does NOT gate position_monitor or intraday's own exit management — a halt must never disable the thing protecting capital already at risk. No frontend banner yet (backend enforcement is the safety-critical part; banner is a nice-to-have follow-up).
4. ✅ **Order seatbelts** (2026-07-20) — `_order_seatbelt_blocked`/`_order_notional_blocked` in structured_runner.py, applied to every order-placement path (BUY + the SELL-closes-a-long path). Duplicate-run guard (one AgentRun places ≤1 order), daily order cap (`max_orders_per_day`, default 20), notional ceiling vs equity (`max_order_notional_pct`, default 40 — matches momentum's own per-name cap). NOTE: the always-on intraday engine's own `broker.submit_bracket_order` call is a SEPARATE code path that does NOT go through these checks (only the kill switch reaches it) — if intraday mode ever gets assigned to a live account again, the seatbelts need their own hook there too.
5. **Deploy discipline** — no deploys during market hours (9:30–16:00 ET) once real users trade; today's mid-market deploy train was acceptable only because arms were brand new. Still a process guideline, not code.
6. ✅ **Post-deploy smoke test** (2026-07-20) — `.github/workflows/post-deploy-smoke.yml`, runs on every push to main. No secrets: polls `/health` through Render's cold-start delay, checks `/auth/login` returns a clean 401 for bogus creds (proves DB+auth alive without storing real credentials in the repo), checks `/api/v1/track-record/` returns valid shaped JSON.
...
New read-only monitoring surface (2026-07-20): `GET /api/v1/monitor/accounts-summary` — own auth (`require_monitoring_key` header, `MONITORING_API_KEY` env, fails closed if unset — NOT the JWT path, built for unattended agents with no login). Per-account equity/24h-change/day-pnl/positions/scan_enabled/today's-run-health from existing equity_snapshot data (no live broker calls). Feeds the hourly cloud routine "TradingAgents Platform Health Monitor" (claude.ai/code/routines/trig_01EheikbuqVa99eGKpCt64LR) — checks backend health + this endpoint + GH Actions status, emails ygc2@njit.edu via the Gmail MCP connector ONLY on a real problem.
...
**NEXT UP**: Monday 2026-07-20 09:35 ET = first fully-healthy live test (verify scans fired for all 3 arms after 09:40; watchdog Action emails on silent days at 16:50). Experiment pipeline: (1) dedicated aggression account — PEAD at 25% sizing/$1B floor (user creates Alpaca paper #2 + platform signup, bounds already raised); (2) momentum forward arm decision — tournament done 2026-07-18 (see header), needs a live engine (`strategy_mode="momentum"`, monthly top-4 rotation, zero LLM) + dedicated paper account IF user wants to forward-test it; judge vs equal-weight benchmark, not absolute return; (3) real-time news arm off Alpaca news WS (forward-only, after a clean forward week); (4) options expression of the PEAD signal — defined-risk call spreads, same trigger, A/B vs stock arm (forward-only, paper-fill caveats). USER TODO (secrets): NEON_DATABASE_URL + BACKUP_PASSPHRASE (nightly backups), ALPACA_API_KEY/SECRET (weekly research Action), SENTRY_DSN on Render (error alerting). DONE since last list: earnings arm live (whole-market), email via Brevo HTTPS, backup workflow committed, scan watchdog Action, momentum tournament + out-of-universe validation, Sentry backend init, weekly research cron Action, rejection_reason CoT strip, resend-verification honesty fix.
