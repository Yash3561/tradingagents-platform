...
> Last updated: 2026-07-17 pre-market (CRITICAL session: scheduled scans had been silently dead since 7/15 — scheduler hardened with timeouts + wide catch-up windows + Redis once-per-day guard + heartbeat logs; earnings blackout breaker was vetoing EVERY PEAD order at the broker stage — engine now exempt; earnings arm LIVE on seemplyai@gmail.com with whole-market NASDAQ-calendar reporter pool, 100-ticker validated watchlist fallback, 10% sizing, scan_enabled=true. Missed live UNH signal 7/16 (+29.8% surprise, +7.4% gap) to the dead scheduler + scan-disabled combo — that near-miss found both bugs.)
