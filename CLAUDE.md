...
**NEXT UP**: Monday 2026-07-20 09:35 ET = first fully-healthy live test (verify scans fired for all 3 arms after 09:40; watchdog Action emails on silent days at 16:50). Experiment pipeline: (1) dedicated aggression account — PEAD at 25% sizing/$1B floor (user creates Alpaca paper #2 + platform signup, bounds already raised); (2) cross-sectional momentum-rotation walk-forward tournament (fully backtestable with existing research infra); (3) real-time news arm off Alpaca news WS (forward-only, after a clean forward week); (4) options expression of the PEAD signal — defined-risk call spreads, same trigger, A/B vs stock arm (forward-only, paper-fill caveats). DONE since last list: earnings arm live (whole-market), email via Brevo HTTPS (`BREVO_API_KEY` — Render blocks all SMTP ports; trade entries/exits/halts + verification/reset emails all work, verified delivered 2026-07-18), backup workflow committed (still needs NEON_DATABASE_URL + BACKUP_PASSPHRASE repo secrets), scan watchdog Action. Remaining low-effort: Sentry, weekly tournament cron, strip model chain-of-thought from risk rejection_reason (backend).
